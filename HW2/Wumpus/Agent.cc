...
Action Agent::AgentTick(Percept& percept)
{
	DebugPrintTick();
...
bool Agent::CreateKnowledge(Percept& percept)
{
...
vector<Action> Agent::CreatePlan()
{
	// return the Plan.
...
vector<AgentCoords> Agent::CreatePlanSearch()
{
...
vector<AgentCoords> Agent::CreatePlanSearchReconstructPath(vector<AgentSearchNode> camefrom, AgentSearchNode current)
{
...
AgentSearchNode Agent::CreatePlanSearchGetCurrentParentInfo(AgentSearchNode node, vector<AgentSearchNode> set)
{
...
bool Agent::CreatePlanSearchInSet(AgentSearchNode node, vector<AgentSearchNode> set)
{
	if (node.GetCoords().x != -1 && node.GetCoords().y != -1)
	{
		if (set.size() > 0)
...
vector<Action> Agent::CreatePlanActions(vector<AgentCoords> coordplan)
{
...
void Agent::DebugPrintCoordPlan(vector<AgentCoords> coordslist)
{
	if (av_b_debugmode)
	{
...
void Agent::DebugPrintActionsPlan()
{
	if (av_b_debugmode)
	{
